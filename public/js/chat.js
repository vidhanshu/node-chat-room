...
const $messagesTemplateOtherUser = document.querySelector(
  "#message-template-other"
).innerHTML;
const $userTemplate = document.querySelector("#user-template").innerHTML;
const $linkTemplate = document.querySelector("#link-template").innerHTML;
const $linkTemplateOther = document.querySelector(
  "#link-template-other"
).innerHTML;
...
$form.addEventListener("submit", (evt) => {
  ...
  socket.emit(
    "sendMessage",
    { message: value, current_user: username },
    (acknowledgement) => {
...
/* share on whats app */
document
  .querySelector("#shareable-link")
  .setAttribute(
    "href",
    `whatsapp://send?text=${location.origin} \nuse the room code - ${room} \nJoin the room and let's have some fun 😉`
  );
